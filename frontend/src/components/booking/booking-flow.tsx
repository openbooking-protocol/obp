...
import { formatDate, formatTime, formatDuration, formatPrice, formatCountdown, dateRangeForDays } from "@/lib/utils/format";
...
export function BookingFlow({ service, provider }: BookingFlowProps) {
  ...
  const handleSelectSlot = useCallback(async (slot: Slot) => {
    ...
    try {
      const hold = await obpApi.slots.hold(slot.id);
      setHoldData(hold);
      setHoldExpired(false);
      setStep("form");
    } catch {
      setError("Could not hold this slot. It may have just been booked. Please try another.");
    }
  }, []);
  ...
  const handleSubmit = async () => {
    ...
    setError("");
    try {
      const newBooking = await obpApi.bookings.create({
  ...
  if (step === "success" && booking) {
    return <BookingSuccess booking={booking} service={service} provider={provider} />;
  }
      ...
      {step === "form" && selectedSlot && holdData && (
        <div>
              ...
              <div className="space-y-1 text-sm text-muted-foreground">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-foreground">
                    {formatDate(selectedSlot.start_time, "MMM d")} at {formatTime(selectedSlot.start_time)}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Duration</span>
                  <span className="font-medium text-foreground">{formatDuration(service.duration_minutes)}</span>
                </div>
                {service.price && (
          ...
          <p className="mt-3 text-xs text-muted-foreground">
            By booking, you agree to the provider&apos;s cancellation policy.
          </p>
        </div>
      )}
    </div>
...
function BookingSuccess({
  ...
  return (
    <div className="text-center">
      ...
      <p className="mb-6 text-muted-foreground">Your booking has been received.</p>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:justify-center">
        <a
          href={bookingCalendarUrl(booking.id)}
          download
